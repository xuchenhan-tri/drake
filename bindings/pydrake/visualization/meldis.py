...
class _ViewerApplet:
    ...
    def on_viewer_draw_deformable(self, message):
        ...
        for i, geom in enumerate(message.geom):
            ...
            self._meshcat.SetTriangleMesh(path=geom_path, vertices=vertices, faces=faces, rgba=rgba)
            self._meshcat.SetTransform(path=link_path, X_ParentPath=pose)
        if self._waiting_for_first_draw_message:
            self._waiting_for_first_draw_message = False
            self._set_visible(True)
    ...
    def _convert_deformable_geom(self, geom):
        """Given an lcmt_viewer_geometry_data, parse it into a tuple of (vertices, faces
        Rgba, RigidTransform) if the geometry type is a MESH"""
        assert geom.type == lcmt_viewer_geometry_data.MESH
        num_verts = int(geom.float_data[0])
        num_faces = int(geom.float_data[1])
        # The first two floats encode the number of vertices and number of triangles
        v_start_index = 2
...
class Meldis:
    ...
    def __init__(self, *, meshcat_port=None):
        ...
        self._subscribe(channel="DRAKE_VIEWER_DRAW_DEFORMABLE",
                        message_type=lcmt_viewer_link_data,
                        handler=viewer.on_viewer_draw_deformable)
