...
class DeformableModel final : public multibody::PhysicalModel<T> {
 public:
  ...
  /** Defines a fixed constraint between a deformable body A a rigid body B.
   Such a fixed constraint is modeled as zero-distance holonomic constraints:
   ...
   @param[in] body_A_id    Id of the deformable body under constraint.
   @param[in] body_B       Rigid body under constraint.
   @param[in] X_WB         The pose of body B's frame in the world frame.
   @param[in] shape        The geometry shape of the rigid body B. If a vertex
                           Pᵢ of A, in its reference configuration, is inside or
                           on the surface of the `shape`, then it is subject to
                           the fixed constraint. The vertex Pᵢ is fixed to the
                           point Qᵢ, where Qᵢ is the point rigidly affixed to B
                           that is coincident with Pᵢ when A is in its reference
                           configuration and B has pose X_WB.
   ...
   QueryObject::ComputeSignedDistanceToPoint(). Supported shapes include Box,
   Capsule, Cylinder, Ellipsoid, HalfSpace, and Sphere.
   @throws std::exception if Finalize() has been called on the multibody plant
   owning this deformable model. */
  MultibodyConstraintId AddFixedConstraint(
      DeformableBodyId body_A_id, const Body<T>& body_B,
      const math::RigidTransform<double>& X_WB, const geometry::Shape& shape,
  ...
  const internal::DeformableRigidFixedConstraintSpec& get_fixed_constraint_spec(
      MultibodyConstraintId id) const {
    DRAKE_THROW_UNLESS(fixed_constraint_specs_.count(id) > 0);
    return fixed_constraint_specs_.at(id);
  }

  /** (Internal use only)  Returns a reference to the all of the fixed
   constraints in this model as a map from MultibodyConstraintId to
   DeformableRigidFixedConstraintSpec. */
  const std::map<MultibodyConstraintId,
                 internal::DeformableRigidFixedConstraintSpec>&
  get_fixed_constraint_specs() const {
    return fixed_constraint_specs_;
  }

  /** (Internal use only)  Returns a reference to the all of the fixed
   constraints registered with the deformable body with the given `id`. */
  const std::vector<MultibodyConstraintId>& get_fixed_constraint_specs(
