...
/* DeformableModel implements the interface in PhysicalModel and provides the
 ...
 // TODO(xuchenhan-tri): Move the next two paragraphs to SG.
...
class DeformableModel final : public multibody::internal::PhysicalModel<T> {
 public:
  ...
  /* Registers a deformable body in `this` DeformableModel with the given
   GeometryInstance. Returns a unique identifier for the added geometry.
  ...
  /* Returns the reference positions of the vertices of the deformable body
   ...
   The reference positions is represented as a VectorX with 3N values where N
  ...
  // TODO(xuchenhan-tri): Move to SceneGraph.
  /*
  ...
  // registered with the system. The i-th body corresponds to the i-th
  Matrix3X.
  // For mesh i, the j-th column contains the value of the x-, y-, and
  // z-positions (measured and expressed in the world frame) of the j-th
  vertex
  // in the i-th mesh from the output port.
 ...
 private:
  ...
  /* Builds a FEM model for the body with `id` with linear tetrahedral elements
   ...
   such as the material model of the body is given by `config`.
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
    class ::drake::multibody::internal::DeformableModel);
