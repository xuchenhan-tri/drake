...
class DeformableFixedConstraintTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    /* Add a fixed constraint so that the bottom (with smallest z coordinate in
     the world frame) vertex of the deformable is fixed to the box. */
    const RigidTransformd X_B0D(Vector3d(0, 0, 0.19));
    model_->AddFixedConstraint(deformable_body_id_, box_body, X_B0D, box,
    ...
    plant_->RegisterCollisionGeometry(plant_->world_body(),
                                      RigidTransformd::Identity(), static_box,
                                      "static_box_collision", proximity_prop);
    plant_->RegisterVisualGeometry(plant_->world_body(),
                                   RigidTransformd::Identity(), static_box,
                                   "static_box_visual", illustration_props);
    /* Add a fixed constraint so that the top (with smallest z coordinate in
     the world frame) vertex of the deformable is fixed to the box. */
    const RigidTransformd X_B1D(Vector3d(0, 0, -0.19));
    model_->AddFixedConstraint(deformable_body_id_, plant_->world_body(), X_B1D,
                               static_box, RigidTransformd::Identity());
    plant_->Finalize();
...
TEST_F(DeformableFixedConstraintTest, SteadyState) {
  ...
  /* Initialize the dynamic box close to its equilibrium pose. */
  RigidTransformd X_WB = RigidTransformd(Vector3d(0, 0, -0.4));
  plant_->SetFreeBodyPose(&mutable_plant_context,
                          plant_->get_body(rigid_body_index_), X_WB);
  ...
  constexpr double kVelocityThreshold = 3e-6;  // unit: m/s.
