...
class DeformableReactionForcesTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    deformable_body_id_ =
        RegisterDeformableSphere(&deformable_model, "deformable_sphere");
  ...
  // Registers a deformable sphere
  DeformableBodyId RegisterDeformableSphere(DeformableModel<double>* model,
                                            std::string name) {
    // Create a deformable sphere geometry
    auto geometry = std::make_unique<GeometryInstance>(
        RigidTransformd(Vector3d(0.0, 0.0, kInitialHeight)),
        std::make_unique<Sphere>(kSphereRadius), name);
    ...
    DeformableBodyId id = model->RegisterDeformableBody(
        std::move(geometry), body_config, kResolutionHint);
  ...
  void VerifyJointReactionForces(const Simulator<double>& simulator) {
    ...
    // The reaction force should be equal to the weight of the deformable object
    const Vector3d expected_force(0.0, 0.0, deformable_weight);
  ...
  // Sphere parameters
  const double kSphereRadius{0.1};   // Radius of the sphere in m
  const double kInitialHeight{0.5};  // Initial height of the sphere in m

  // Simulation parameters
  const double kTimeStep{0.001};        // Time step in s
  const double kSimulationTime{10.0};    // Maximum simulation time in s
...
TEST_F(DeformableReactionForcesTest, ReactionForcesMatchDeformableWeight) {
