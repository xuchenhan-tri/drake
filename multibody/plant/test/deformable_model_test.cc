...
TEST_F(DeformableModelTest, AddFixedConstraint) {
  ...
  EXPECT_EQ(deformable_model_ptr_->get_fixed_constraint_specs().size(), 1);
  EXPECT_EQ(
      deformable_model_ptr_->get_fixed_constraint_specs(deformable_id).size(),
      1);
  const DeformableRigidFixedConstraintSpec& spec =
      deformable_model_ptr_->get_fixed_constraint_spec(constraint_id);
