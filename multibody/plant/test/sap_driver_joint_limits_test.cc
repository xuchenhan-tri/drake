...
TEST_F(KukaIiwaArmTests, CouplerConstraints) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (i < 2) {
      ...
      const MatrixXd J_expected =
          (VectorXd::Unit(kNumJoints, left_index) -
           kCouplerGearRatio * VectorXd::Unit(kNumJoints, right_index))
              .transpose();
      EXPECT_EQ(constraint->first_clique_jacobian().MakeDenseMatrix(), J_expected);
    } else {
      ...
      EXPECT_EQ(constraint->first_clique_jacobian().MakeDenseMatrix(), J0_expected);
      EXPECT_EQ(constraint->second_clique_jacobian().MakeDenseMatrix(), J1_expected);
    }
