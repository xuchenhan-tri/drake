...
MultibodyConstraintId DeformableModel<T>::AddFixedConstraint(
  ...
  /* There's no easy way to determine if the given `body_B` is registered with
   the MbP owning `this` deformable model, but here we check the index and the
   name to guard against potential mismatches. */
  if (&plant_->get_body(body_B.index()) != &body_B ||
      body_B.name() != plant_->get_body(body_B.index()).name()) {
  ...
  internal::DeformableRigidFixedConstraintSpec spec{
      body_A_id, body_B.index(), {}, {}};
  const RigidTransformd X_WG = X_WB * X_BG;
  geometry::SceneGraph<double> scene_graph;
  geometry::SourceId source_id = scene_graph.RegisterSource("deformable_model");
  auto instance =
      std::make_unique<GeometryInstance>(X_WG, shape.Clone(), "rigid shape");
  ...
  auto query =
      scene_graph.get_query_output_port().Eval<geometry::QueryObject<double>>(
          *context);
  const VectorX<T> p_WP = GetReferencePositions(body_A_id);
  const RigidTransformd X_BW = X_WB.inverse();
  for (int i = 0; i < p_WP.size() / 3; ++i) {
    const Vector3<T>& p_WPi = p_WP.template segment<3>(3 * i);
    const std::vector<geometry::SignedDistanceToPoint<T>> signed_distances =
        query.ComputeSignedDistanceToPoint(p_WPi);
    DRAKE_DEMAND(ssize(signed_distances) == 1);
    const T& signed_distance = signed_distances[0].distance;
    if (signed_distance <= 0.0) {
      spec.vertices.push_back(i);
      /* Qi is conincident with Pi. */
      const Vector3<T>& p_WQi = p_WPi;
      spec.p_BQs.emplace_back(X_BW * p_WQi);
    }
  }
  if (spec.p_BQs.empty()) {
    // Returns an invalid id if no constraint is actually added.
    // TODO(xuchenhan-tri): document and test this behavior.
    return MultibodyConstraintId();
  }
