...
DeformableBodyId DeformableModel<T>::RegisterDeformableBody(
  ...
  // TODO(xuchenhan-tri): Verify required properties are specified. For example,
  /*
   const ProximityProperties* proximity = proximity_properties();
   // Must have friction coefficients specified.
   DRAKE_DEMAND(proximity != nullptr);
   DRAKE_THROW_UNLESS(proximity.HasProperty(geometry::internal::kMaterialGroup,
...
systems::DiscreteStateIndex DeformableModel<T>::GetDiscreteStateIndex(
  ...
  if (discrete_state_indexes_.find(id) == discrete_state_indexes_.end()) {
    throw std::logic_error(fmt::format(
        "No deformable body with id: {} has been registered.", to_string(id)));
  }
...
const fem::FemModel<T>& DeformableModel<T>::GetFemModel(
  ...
  if (fem_models_.find(id) == fem_models_.end()) {
    throw std::logic_error("Invalid body id: " + to_string(id));
  }
...
const VectorX<T>& DeformableModel<T>::GetReferencePositions(
    DeformableBodyId id) const {
  if (reference_positions_.find(id) == reference_positions_.end()) {
    throw std::logic_error(fmt::format(
        "No deformable body with id: {} has been registered.", to_string(id)));
  }
...
void DeformableModel<T>::DoDeclareSystemResources(MultibodyPlant<T>* plant) {
  ...
  // TODO(xuchenhan-tri): Move the output port to SG.
  /*
