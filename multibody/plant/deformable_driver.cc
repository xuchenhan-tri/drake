...
DeformableDriver<T>::DeformableDriver(
  ...
  velocity_integrator_ =
      std::make_unique<fem::internal::VelocityNewmarkScheme<T>>(
...
void DeformableDriver<T>::AppendDiscreteContactPairs(
  ...
  for (DeformableBodyIndex index(0); index < deformable_model_->num_bodies();
    ...
    for (int i = 0; i < deformable_rigid_contact.num_contact_points(); ++i) {
      const geometry::GeometryId rigid_geometry_id =
          deformable_rigid_contact.rigid_ids()[i];
      ...
      const Vector3<T>& p_WC = deformable_rigid_contact.contact_points_W()[i];
      ...
      const Vector3<T>& nhat_BA_W = -deformable_rigid_contact.nhats_W()[i];
...
void DeformableDriver<T>::AppendContactKinematics(
  ...
  if constexpr (!std::is_same_v<T, double>) {
    ...
    for (DeformableBodyIndex index(0); index < deformable_model_->num_bodies();
      ...
      const int num_trees = manager_->tree_topology().num_trees();
      const int deformable_clique_index = num_trees + index;
      ...
      for (int i = 0; i < deformable_rigid_contact.num_contact_points(); ++i) {
        ...
        const GeometryId rigid_geometry_id =
            deformable_rigid_contact.rigid_ids()[i];
...
void DeformableDriver<T>::CalcFreeMotionFemState(
  ...
  if constexpr (std::is_same_v<T, double>) {
    ...
    const FemSolver<T> solver(&model, velocity_integrator_.get());
...
void DeformableDriver<T>::CalcNextFemState(const systems::Context<T>& context,
  ...
  if constexpr (std::is_same_v<T, double>) {
    ...
    if (contact_data.num_contact_points() == 0) {
      ...
      const StackedVectors<T>& stacked_v_stars =
          EvalParticipatingFreeMotionVelocities(context);
      const int offset = stacked_v_stars.offset(index);
      const int size = stacked_v_stars.size(index);

      const VectorX<T> participating_v_star = stacked_v_stars.vector(index);
      const VectorX<T> participating_v_next =
          stacked_v_next.segment(offset, size);
      ...
      velocity_permutation.ApplyInverse(permuted_dv, &dv);
      const VectorX<T>& v_star =
          EvalFreeMotionFemState(context, index).GetVelocities();
      const VectorX<T> v_next = v_star + dv;
      const FemState<T>& fem_state = EvalFemState(context, index);
      velocity_integrator_->AdvanceOneTimeStep(fem_state, v_next,
                                               next_fem_state);
    }
  } else {
...
void DeformableDriver<T>::CalcFreeMotionTangentMatrix(
  ...
  if constexpr (!std::is_same_v<T, double>) {
    ...
    fem_model.CalcTangentMatrix(
        state,
        manager_->plant().time_step() * velocity_integrator_->GetWeights(),
        tangent_matrix);
  }
}
