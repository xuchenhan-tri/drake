...
Block3x3SparseMatrix<double> MakeBlockSparseMatrix() {
  ...
  triplets.emplace_back(2, 1, MakeMatrix(2.0));
  triplets.emplace_back(3, 2, MakeMatrix(3.0));
...
GTEST_TEST(MatrixBlockTest, MultiplyAndAddTo) {
  ...
  VectorXd y1 = VectorXd::LinSpaced(12, 1.0, 2.0);
  VectorXd y2 = y1;
...
GTEST_TEST(MatrixBlockTest, LeftMultiplyByBlockDiagonal) {
  ...
  for (int i = 0; i < num_Gs; ++i) {
    Gs.emplace_back(MakeMatrix(3.14 * i));
  }
  const int start = 2;
  const int end = 5;
...
GTEST_TEST(MatrixBlockTest, MultiplyWithScaledTransposeAndAddTo) {
  ...
  VectorXd scale = VectorXd::LinSpaced(9, 0.0, 1.0);
  MatrixXd y1 = MatrixXd::Zero(12, 12);
  y1(0, 1) = 0.72;
  y1(4, 4) = 0.172;
  MatrixXd y2 = y1;
...
GTEST_TEST(MatrixBlockTest, StackDenseMatrix) {
  ...
  std::vector<Block3x3SparseMatrix<double>::Triplet> triplets;
  triplets.emplace_back(0, 0, MakeMatrix(4.0));
  triplets.emplace_back(1, 1, MakeMatrix(5.0));
