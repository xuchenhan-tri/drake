...
/* Data structure to store individual non-zero blocks of BlockSparseMatrix.
 ...
 constructed. Once constructed, the MatrixBlock cannot change from a dense
 representation to a sparse one or vice versa. This class provides a series of
 matrix computation functions that exploits the sparsity if the MatrixBlock is
 sparse and falls back to dense algebra otherwise. We use `M` to denote `this`
 MatrixBlock throughout this class.
 @tparam_default_scalar */
template <class T>
class MatrixBlock {
 public:
  ...
  /* Computes G*M where G is a block diagonal matrix with the diagonal blocks
   ...
   are {Gs[start], ..., Gs[end]}. The result is returned as a MatrixBlock, dense
   if M is dense, sparse otherwise.
  ...
  /* Returns the MatrixBlock as an Eigen dense matrix. Useful for debugging. */
