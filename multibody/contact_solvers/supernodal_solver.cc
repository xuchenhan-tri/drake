...
void ComputeWeightMatrixTerms(
  ...
  for (size_t i = 0; i < jacobian_row_data.size(); ++i) {
    ...
    for (size_t j = 0; j < jacobian_row_data.size(); ++j) {
      ...
      J.TransposeMultiplyAndAddTo(GJ, &block);
      c_offset += GJ.cols();
    }
    r_offset += J.cols();
  }
...
class SuperNodalSolver::CliqueAssembler final
 ...
 public:
  ...
  void Initialize(std::vector<JacobianBlock<double>>&& jacobian_row);

 private:
  void SetDenseData() override;

 private:
  std::vector<JacobianBlock<double>> jacobian_row_data_;
...
void SuperNodalSolver::Initialize(
  ...
  for (size_t i = 0; i < cliques.size(); ++i) {
    ...
    std::vector<JacobianBlock<double>> jacobian_blocks_of_row;
...
void SuperNodalSolver::CliqueAssembler::Initialize(
    std::vector<JacobianBlock<double>>&& r) {
  int num_vars = 0;
