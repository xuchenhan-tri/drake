...
#else
  const BlockSparseLinearOperator<T> A_op("Tangent matrix", &A);
  const SystemDynamicsData<T> dynamics_data(&A_op, nullptr, &v_star);
#endif

  const VectorX<T>& participating_v0 = EvalParticipatingVelocities(context);
  VectorX<T> vc0(Jc.rows());
  Jc.Multiply(participating_v0, &vc0);
  const PointContactData<T> contact_data(&point_data.phi0, &vc0, &Jc_op,
