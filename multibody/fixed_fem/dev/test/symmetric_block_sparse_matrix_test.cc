...
std::unique_ptr<SymmetricBlockSparseMatrix<double>> MakeBlockSparseMatrix() {
  ...
  auto A_blocks = std::make_unique<SymmetricBlockSparseMatrixImpl<double, 3>>(
      std::move(row_blocks));
  A_blocks->AddToBlock(0, 0, A00);
  A_blocks->AddToBlock(1, 1, A11);
  A_blocks->AddToBlock(1, 2, A12);
...
GTEST_TEST(SymmetricBlockSparseMatrixTest, Construction) {
  ...
  const auto A_blocks = MakeBlockSparseMatrix();
...
GTEST_TEST(SymmetricBlockSparseMatrixTest, Zero) {
  auto A_blocks = MakeBlockSparseMatrix();
  A_blocks->SetZero();
  EXPECT_TRUE(
