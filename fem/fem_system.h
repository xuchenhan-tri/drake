...
class FemSystem final : public systems::LeafSystem<T> {
 public:
  ...
  FemSystem(T dt) : parser_(this), dt_(dt) {}
  ...
  void AdvanceOneTimeStep(const systems::Context<T>& context,
    ...
    Matrix3X<T> q(Q_.rows(), Q_.cols());
    for (int i = 0; i < q.cols(); ++i) {
      // Let (X,Y,Z) be the undeformed position of the node, and the (x,y,z) be
      // the current position of the node. Then, x = X, y = Y and z = Z +
      // wave_magnitude * sin (wave_frequency*(X + wave_speed * time)).
      q(2, i) = Q_(2, i) +
                wave_magnitude_ *
                    std::sin(wave_frequency_ * (wave_speed_ * time + Q_(0, i)));
    }
    const VectorX<T>& q_vec =
        Eigen::Map<VectorX<T>>(q.data(), q.rows() * q.cols());
    next_states->get_mutable_vector().SetFromVector(q_vec);
  }

  void set_dt(T dt) { dt_ = dt; }
  T get_dt() const { return dt_; }
  void set_wave_speed(T speed) { wave_speed_ = speed; }
  void set_wave_frequency(T frequency) { wave_frequency_ = frequency; }
  void set_wave_magnitude(T magnitude) { wave_magnitude_ = magnitude; }
 ...
 private:
  ...
  Matrix3X<T> Q_;
  T wave_speed_{0.2};
  T wave_frequency_{30.0};
  T wave_magnitude_{0.03};
  T dt_{0.1};
