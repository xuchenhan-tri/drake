...
class Quadrature {
 public:
  ...
  template <long unsigned int NumQuads>
  Quadrature(std::array<VectorD, NumQuads> points,
             std::array<T, NumQuads> weights)
  ...
  decltype(auto) GetPoints() {
    ...
    if constexpr (NaturalDimension == 2) {
      ...
      if constexpr (QuadratureOrder == 1) {
        ...
        //  (1/3, 1/3)                     1.0
        std::array<VectorD, 1> points = {{{1.0 / 3.0, 1.0 / 3.0}}};
        return points;
      ...
      if constexpr (QuadratureOrder == 1) {
        // quadrature point location,  weight/area
        //  (1/4, 1/4, 1/4)                1.0
        std::array<VectorD, 1> points = {{{0.25, 0.25, 0.25}}};
        return points;
      } else if constexpr (QuadratureOrder == 2) {
        // quadrature point location,  weight/area
        //  (a, b, b)                      1/4
  ...
  decltype(auto) GetWeights() {
    ...
    if constexpr (NaturalDimension == 2) {
      ...
      if constexpr (QuadratureOrder == 1) {
        ...
        //  (2/3, 1/6)                     1/3
        //  (1/6, 2/3)                     1/3
        std::array<T, 3> weights = {{1.0 / 6.0, 1.0 / 6.0, 1.0 / 6.0}};
        return weights;
      } else if constexpr (QuadratureOrder == 3) {
        // quadrature point location,  weight/area
        //  (1/3, 1/3)                     -9/16
