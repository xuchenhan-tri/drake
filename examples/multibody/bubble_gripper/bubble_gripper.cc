...
DEFINE_double(bubble_E, 5e4, "Young's modulus of the deformable body [Pa].");
DEFINE_double(bubble_nu, 0.45,
              "Poisson's ratio of the deformable body, unitless.");
DEFINE_double(bubble_density, 1000,
...
int do_main() {
  ...
  const double kHydroModulus = 5e6;
  ...
  const RigidTransformd X_WB1(RollPitchYawd(1.57, 0, 0),
  ...
  /* Pose of the left finger body (at initialization) in the world frame. */
  const RigidTransformd X_WL = RigidTransformd(math::RollPitchYawd(0, 1.57, 0),
                                               Vector3d(-0.0725, -0.065, 0.06));
  /* All deformable points inside this box will be subject to fixed constraints.
   */
  Box box(0.5, 0.01, 0.5);
  deformable_model->AddFixedConstraint(
      left_bubble_id, left_finger, X_WL, box,
      /* X_LG: the pose of the box in the left finger's frame. */
      RigidTransformd(Vector3d(0.0, -0.03, 0)));

  auto right_bubble_mesh = std::make_unique<Mesh>(bubble_vtk);
  /* Pose of the right bubble (at initialization) in the world frame. */
  const RigidTransformd X_WB2(RollPitchYawd(-1.57, 0, 0),
                              Vector3d(-0.185, 0.095, 0.06));
  ...
  const RigidTransformd X_WR = RigidTransformd(math::RollPitchYawd(0, 1.57, 0),
                                               Vector3d(-0.0725, 0.065, 0.06));
  deformable_model->AddFixedConstraint(
      right_bubble_id, right_finger, X_WR, box,
      /* X_LG: the pose of the box in the right finger's frame. */
      RigidTransformd(Vector3d(0.0, 0.03, 0)));
