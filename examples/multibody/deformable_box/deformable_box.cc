...
DEFINE_double(k, 1e12, "Contact stiffness");
...
using multibody::internal::DeformableBodyId;
...
int do_main() {
  ...
  geometry::AddContactMaterial({}, FLAGS_k, surface_friction, &proximity_props);
  ...
  illus_prop.AddProperty("phong", "diffuse",
                         Eigen::Vector4d(0.7, 0.5, 0.4, 0.5));
  ...
  const DeformableBodyId box_id = deformable_model->RegisterDeformableBody(
      std::move(box_instance), deformable_config, kRezHint);
  ...
  /* The force magnitude in the positive z direction to compensate for
  gravity
   of the system. */
  ...
  /* Here we are use the same Sine source to:
    1. Generate a horizontal harmonic forcing of the finger with the
    prescribed
  ...
  /* Add up the constant force and the harmonic force source and supply the
  sum
  ...
  // /* Set initial conditions for the box. */
  // auto& plant_context =
  //     diagram->GetMutableSubsystemContext(plant, diagram_context.get());
  // deformable_model_ptr->ScaleAndRigidTransform(
  //     box_id, 1.0, math::RigidTransformd::Identity(), &plant_context);
  unused(box_id);
  ...
  /* Set the initial position of the gripper to be of the same width as
  the
   deformable box. */
