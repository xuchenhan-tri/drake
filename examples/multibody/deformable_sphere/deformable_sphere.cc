...
int do_main() {
  ...
  /* Set up a ground. */
  Box ground{4, 4, 4};
  const RigidTransformd X_WG(Eigen::Vector3d{0, 0, -1.9});
  ...
  const std::string sphere_vtk = FindResourceOrThrow(
      "drake/examples/multibody/deformable_sphere/sphere_1k.vtk");
  auto sphere_mesh = std::make_unique<Mesh>(sphere_vtk, 1.0);
  const RigidTransformd X_WB(Vector3<double>(0.0, 0.0, 1.0));
