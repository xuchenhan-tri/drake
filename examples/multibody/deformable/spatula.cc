...
DEFINE_double(simulation_time, 35.0, "Desired duration of the simulation [s].");
...
int do_main() {
  ...
  const CoulombFriction<double> surface_friction(3.0, 3.0);
  ...
  /* Set up a ground. */
  Box ground{1, 1, 1};
  const RigidTransformd X_WG(Eigen::Vector3d{0, 0, -0.60});
  ...
  parser.AddModelsFromUrl(
      "package://drake/examples/multibody/deformable/models/spatula.sdf");
  RigidTransformd X_WS(RollPitchYawd(0, 0, -M_PI / 2.0),
                       Vector3d(-0.33, 0.0, 0.015));
  plant.SetDefaultFreeBodyPose(plant.GetBodyByName("spatula"), X_WS);

  IllustrationProperties force_application_point_props;
  force_application_point_props.AddProperty("phong", "diffuse",
                                            Vector4d(0.15, 0.90, 0.65, 0.9));
  const Vector3d p_BoBq_B(0.0, -0.02, 0);
  const RigidTransformd X_SV(RollPitchYawd(0, 0, 0), p_BoBq_B);
  plant.RegisterVisualGeometry(plant.GetBodyByName("spatula"), X_SV,
                               Sphere(0.01), "force_application_point",
                               force_application_point_props);
  ...
  const double open_width = 0.012;
  const double closed_width = open_width;
  const double lifted_height = 0.0;
  ...
  builder.Connect(external_force_source.get_output_port(0),
