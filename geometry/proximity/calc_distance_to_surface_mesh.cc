...
/* Computes the distance from the point Q to the non-degenerate line segment AB
 (the length of AB is positive). */
double CalcDistanceToLineSegment(const Vector3<double>& Q,
  ...
  if (AQ.dot(AB) <= 0.0) return AQ.norm();

  Vector3<double> BQ = Q - B;

  // If the projection of Q on the line AB is equal to B + t * AB for t > 0,
  // then the shortest distance is BQ.
  if (BQ.dot(AB) >= 0.0) return BQ.norm();

  // The projection of Q on the line AB in the line segment AB.
  return (AB.cross(AQ)).norm() / AB.norm();
}

/* Computes the distance from the point p_WQ to a non-degenerate triangle t_W.
 */
double CalcDistanceToTriangle(const Vector3<double>& p_WQ, Triangle t_W) {
  ...
  const Vector3<double> zhat_W = p_AB_W.cross(p_AC_W).normalized();
  const Vector3<double> xhat_W = p_AB_W.normalized();
  RotationMatrix<double> R_TW = RotationMatrix<double>::MakeFromOrthonormalRows(
      xhat_W, zhat_W.cross(xhat_W), zhat_W);
  ...
  const double d_AB = CalcDistanceToLineSegment(p_TQ, p_TA, p_TB);
  const double d_AC = CalcDistanceToLineSegment(p_TQ, p_TA, p_TC);
  const double d_BC = CalcDistanceToLineSegment(p_TQ, p_TB, p_TC);

  return std::min({d_AB, d_AC, d_BC});
}
